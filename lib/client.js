...
Client.prototype._defineMethod = function(method, location) {
  ...
  return function(args, callback, options) {
    ...
    return self._invoke(method, args, location, callback, options);
  };
};
...
Client.prototype._invoke = function(method, args, location, callback, options) {
  ...
  http.request(location, xml, function(err, response, body) {
    ...
    if (err) {
      if (!callback) defer.reject(err); else callback(err);
    } else {
      try {
        ...
        error.body = body;
        if (!callback) defer.reject(error); else return callback(error, response, body);
      }
      ...
      if (!callback) defer.resolve(result); else callback(null, result, body);
    }
  }, headers, options);
  if (!callback) return defer.promise;
};
